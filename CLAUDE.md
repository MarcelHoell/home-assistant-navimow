...
- **The OAuth callback view is registered once**, guarded by a flag in `hass.data[DOMAIN]`, and only acts on a flow that is currently waiting. `pick_pending_flow()` lives in `const.py` so it can be tested without Home Assistant. The state-less fallback is deliberate: it is unverified whether Segway echoes `state`. If the "did not echo the OAuth state parameter" warning never shows up in practice, delete the fallback.
